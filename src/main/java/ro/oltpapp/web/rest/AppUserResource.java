...
/**
 * REST controller for managing {@link ro.oltpapp.domain.AppUser}.
 */
@RestController
@RequestMapping("/api")
public class AppUserResource {
    ...
    public ResponseEntity<AppUserDTO> createAppUser(@Valid @RequestBody AppUserDTO appUserDTO) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/app-users/" + result.getIdUser()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getIdUser().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /app-users/:idUser} : Updates an existing appUser.
     *
     * @param idUser the id of the appUserDTO to save.
    ...
    public ResponseEntity<AppUserDTO> updateAppUser(
        ...
        log.debug("REST request to update AppUser : {}, {}", idUser, appUserDTO);
        if (appUserDTO.getIdUser() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idUser, appUserDTO.getIdUser())) {
            throw new BadRequestAlertException("Invalid ID", ENTITY_NAME, "idinvalid");
        }

        if (!appUserRepository.existsById(idUser)) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, appUserDTO.getIdUser().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /app-users/:idUser} : Partial updates given fields of an existing appUser, field will ignore if it is null
     *
     * @param idUser the id of the appUserDTO to save.
     * @param appUserDTO the appUserDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated appUserDTO,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/app-users/{idUser}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<AppUserDTO> partialUpdateAppUser(
        @PathVariable(value = "idUser", required = false) final Long idUser,
        @NotNull @RequestBody AppUserDTO appUserDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update AppUser partially : {}, {}", idUser, appUserDTO);
        if (appUserDTO.getIdUser() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idUser, appUserDTO.getIdUser())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, appUserDTO.getIdUser().toString())
        );
    }
